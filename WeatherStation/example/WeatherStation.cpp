...
Sensor* Sensor::creatObject( uint8_t ID, uint8_t pin, uint8_t auxiliaryPin)
{
    ...
    switch ( ID )
    {
        ...
        case FUNCTION_SENSOR_BMP280:
            pObject = new BMP280Sensor( pin );
            break;
...
/****/
BMP280Sensor::BMP280Sensor(uint8_t pin)
{
    attachPin = pin;
    pDriver = new DFRobot_BMP280();
}

void BMP280Sensor::start( )
{
    Wire.begin();
    pDriver->init();
    pDriver->begin(0x77, 0x58);

    pressureValue = 32767;
}

void BMP280Sensor::stop( )
{

}

boolean BMP280Sensor::run()
{
    int32_t value;


    //pDriver->getPressure( &value );
    value = pDriver->readPressureValue();

    value = (value + 50)/100;  //Pa -> hPa

    pressureValue = ( value > 32767 ) ? INVALID_INT16 : (int16_t)value;

    return true;
}

int16_t BMP280Sensor::getValue( uint8_t dataType )
{
    return pressureValue;
}


String BMP280Sensor::formatValue( int16_t value, uint8_t dataType )
{
...
const char * BMP280Sensor::getDataName( uint8_t dataType )
{
    return "BARO";
}
